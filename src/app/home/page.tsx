...
export default function Home() {
  ...
  const [filters, setFilters] = useState<FilterOptions>({
    departmentId: userData.data?.user.departmentId?.toString() ?? "0",
  ...
  const handleSelect = (name: string, value: HandleSelectValue) => {
    if (name === "department" && typeof value === "object" && value !== null) {
      ...
      setFilters((prev) => ({
        ...prev,
        departmentId: departmentValue.id, // Use the explicit type
    ...
    } else if (name === "quarter" && typeof value === "string") {
      setFilters((prev) => ({
        ...prev,
        [name]: value, // Quarter is a string
      }));
    }
  };
  return (
    <div className="mt-20 flex flex-col gap-4 p-2">
      <div>
