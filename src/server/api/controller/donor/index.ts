...
import { donorMasterInFinanceProject } from "~/server/db/schema";
...
export const getDonors = protectedProcedure.input(z.object({
    ...
    const searchCondition = searchTerm
      ? ilike(donorMasterInFinanceProject.name, `%${searchTerm}%`)
      : undefined;

    // Fetch the donor records with pagination, search, and offset
    const donors = await ctx.db.query.donorMasterInFinanceProject.findMany({
    ...
    const totalCountResult = await db.select({ count: count() })
      .from(donorMasterInFinanceProject)
