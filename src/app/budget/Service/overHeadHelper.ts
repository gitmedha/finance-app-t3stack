...
import { getBaseStructure, months } from "../Constants/overHeadConstants";
...
export const recalculateTotals = (
  tableData: TableData,
  setTotalQty: (totals: totalschema) => void,
) => {
  ...
  Object.values(tableData).forEach((data) => {
    ...
    newTotals.totalQ1 +=
      Number(data.april ?? 0) + Number(data.may ?? 0) + Number(data.june ?? 0);
